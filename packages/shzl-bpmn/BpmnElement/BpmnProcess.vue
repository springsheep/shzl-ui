<template>
   <div>
            ...
            <el-collapse-item name="1">
               ...
               <el-form-item label="流程分类" prop="processCategory">
                  <el-select v-model="formData.processCategory">
                     <el-option v-for="(item, index) in this.processCategory" :key="index" :label="item.name" :value="item.id"></el-option>
                  </el-select>
               </el-form-item>
            </el-collapse-item>
      ...
      <properties
         ...
         :element="element"
         :modeler="modeler"
         @changePropertiesDrawer="changePropertiesDrawer"
...
export default {
   ...
   created() {
      ...
      this.executionListenerLength =
         this.element.businessObject.extensionElements?.values.filter((item) => item.$type === this.descriptorPrefix + 'ExecutionListener').length ??
         0;
      this.propertiesLength =
         this.element.businessObject.extensionElements?.values.filter((item) => item.$type === this.descriptorPrefix + 'Properties').shift()?.values
            .length ?? 0;
   },
   methods: {
