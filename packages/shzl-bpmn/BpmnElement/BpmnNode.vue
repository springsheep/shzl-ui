            ...
            <el-collapse-item name="1">
               ...
               <el-form-item v-if="!!showConfig.taskCategory" label="任务分类" prop="taskCategory">
                  <el-select v-model="formData.category">
                     <el-option v-for="(item, index) in this.taskCategory" :key="index" :label="item.name" :value="item.id"></el-option>
                  </el-select>
               </el-form-item>
...
export default {
   ...
   created() {
      ...
      this.taskListenerLength =
         this.element.businessObject.extensionElements?.values.filter((item) => item.$type === this.descriptorPrefix + 'TaskListener').length ?? 0;
   },
   methods: {
